...
public class OlMoneyServiceImpl implements OlMoneyService {
    ...
    @Override
    public List<MoneyDto> queryMoneyListByAccountId(Integer accountId) {

        if (ObjectUtils.isEmpty(accountId)){
            throw new OlRuntimeException(Ol_USER_ERROR_1703);
        }
        List<MoneyEntity> test = olMoneyMapper.selectOpeMoneyTime(accountId);
        if (test.size() == 0){
            throw new OlRuntimeException(Ol_USER_ERROR_1602);
        }
        return OlBeanUtils.createCopy(olMoneyMapper.selectOpeMoneyTime(accountId), MoneyDto.class);
    }
}
